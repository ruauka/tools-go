...
// StringToFloat64BestEffort parses floating-point number s.
//
// It is equivalent to strconv.ParseFloat(s, 64), but is faster.
//
// 0 is returned if the number cannot be parsed.
func StringToFloat64BestEffort(s string) float64 {
	if len(s) == 0 {
		return 0
	}
	i := uint(0)
...
// StringToFloat64 parses floating-point number s.
//
// It is equivalent to strconv.ParseFloat(s, 64), but is faster.
//
// See also StringToFloat64BestEffort.
